    ...
    if (parent) {
      ...
      if (currentFilter.includes(' and ')) {
        ...
        currentList.forEach((elem) => {
          if (!(elem.includes('PK') || elem.includes('Año') || elem.includes('V') || elem.includes('NACVS') || elem.includes('Carretera') || elem.includes('NACV') || elem.includes('NACVM'))) {
            if (elem.includes("', '")) {
              if (elem.includes(`'${parent}', `)||elem.includes(`, '${parent}'`)) {
                let finalElem ;
                if(elem.includes(`'${parent}', `)){
                  finalElem  = elem.replace(`'${parent}', `, '');
                }else if(elem.includes(`, '${parent}'`)){
                  finalElem  = elem.replace(`, '${parent}'`, '');
                }
        ...
        nElem = `(${nElem})`;                            // paréntesis equilibrados
        if (!/IN\s*\(/i.test(nElem)) {
          nElem = nElem.replace(/IN\s+/i, 'IN (') + ')'; // sólo si hace falta abrir/cerrar
        }
