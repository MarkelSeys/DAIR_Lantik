...
/** IMPORTANTE:
 *  Con columnas frozen, Tabulator crea dos holders.
 *  Necesitamos el holder NO frozen (donde está la tabla "scrollable").
 *  OJO: según versión puede ser .tabulator-frozen-left/right.
 */
function resolveNonFrozenHolder(tabEl: HTMLElement | null): HTMLElement | null {
  if (!tabEl) return null
  const holders = Array.from(tabEl.querySelectorAll<HTMLElement>('.tabulator-tableholder, .tabulator-tableHolder'))
  if (!holders.length) return null

  const FROZEN_WRAP_SEL = '.tabulator-frozen, .tabulator-frozen-left, .tabulator-frozen-right'

  // 1) el que NO está dentro del contenedor frozen
  const nonFrozen = holders.find(h => !h.closest(FROZEN_WRAP_SEL))
  if (nonFrozen) return nonFrozen

  // 2) fallback: el que tenga celdas no frozen
  const byCell = holders.find(h => h.querySelector('.tabulator-cell:not(.tabulator-frozen)'))
  if (byCell) return byCell

  // 3) fallback: el más “scrollable”
  holders.sort((a, b) => (b.scrollWidth - b.clientWidth) - (a.scrollWidth - a.clientWidth))
...
const CustomFactorPkPivotTable = (props: AllWidgetProps<any>) => {
  ...
  const nonFrozenHolderRef = React.useRef<HTMLElement | null>(null)
  ...
  const holderCleanupRef = React.useRef<null | (() => void)>(null)
  ...
  React.useEffect(() => {
    ...
    // limpiar listener del holder anterior (si existía)
    holderCleanupRef.current?.()
    holderCleanupRef.current = null

    if (tabRef.current) {
      tabRef.current.destroy()
      tabRef.current = undefined
    }
    nonFrozenHolderRef.current = null
    ...
    const tab = new Tabulator(tableMountRef.current, {
      ...
      renderComplete: () => {
        ...
        requestAnimationFrame(() => {
          ...
          const holder = resolveNonFrozenHolder(tabEl)
          nonFrozenHolderRef.current = holder

          // aplicar scroll actual SOLO a la parte no frozen
          if (holder) holder.scrollLeft = scrollXRef.current

          // reenganchar listener del holder (por si Tabulator recrea DOM)
          holderCleanupRef.current?.()
          holderCleanupRef.current = null

          const bar = hScrollRef.current
          const chartInner = chartRightInnerRef.current
          if (!holder || !bar || !chartInner) return
          ...
          holderCleanupRef.current = () => holder.removeEventListener('scroll', onHolderScroll as any)
        })
      }
    })
  ...
  // scrollbar compartido: ajustar ancho + aplicar scroll a tabla (SOLO parte no frozen) + chart
  React.useEffect(() => {
    const bar = hScrollRef.current
    const inner = hScrollInnerRef.current
    if (!bar || !inner) return
    ...
    if (nonFrozenHolderRef.current) nonFrozenHolderRef.current.scrollLeft = x
  ...
  // scroll coordinado (barra -> chart + SOLO columnas de datos)
  React.useEffect(() => {
    const bar = hScrollRef.current
    const chartInner = chartRightInnerRef.current
