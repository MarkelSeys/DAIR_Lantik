...
export default function Widget (props: AllWidgetProps<unknown>) {
  ...
  function setChartCategoryAndColors (widgetId: string, fieldName: string, attUnique: any) {
      ...
      const fromWin: string[] = Array.isArray((window as any).attUnique)
        ? (window as any).attUnique.map((s: any) => String(s))
        : []
      ...
      const allValues = Array.from(
        new Set([...fromWin, ...fromAtt, ...fromBase].map(s => String(s)))
      )
    
      // 3) seleccionados (3 en tu ejemplo)
      const seleccionados: string[] = (window as any).valoresAgg || []
      const setSel = new Set(seleccionados.map(s => String(s)))
    
      const GRIS_RESTO = '#dcdcdc'
      const colorMapFinal: Record<string, string> = {}
    
      // helper: busca color en baseColorMap aunque cambien espacios/mayúsculas
      const findRendererColor = (name: string): string | undefined => {
        const trimmed = name.trim().toLowerCase()
        const key = Object.keys(baseColorMap).find(
          k => k.trim().toLowerCase() === trimmed
        )
  ...
  ;(window as any).valoresSeleccionados = allValues
 
  store.dispatch(appActions.widgetStatePropChange(widgetId, 'categoryFieldOverride', fieldName))
  store.dispatch(appActions.widgetStatePropChange(widgetId, 'categoryColorsOverride', colorMapFinal))
  store.dispatch(appActions.widgetStatePropChange(widgetId, 'categoryValuesOverride', allValues))
}
