...
export default function WidgetUnido (props: AllWidgetProps<any>) {
  ...
  const [history, setHistory] = useState<Dict<Set<string>>[]>([])
  const historyRef = useRef<Dict<Set<string>>[]>([])
  ...
  const handleUndo = async () => {
    ...
    if (currentHistory.length < 2) return // nada que deshacer
            ...
            : (
              <>
                <span style={styles.badge}>
                  Categorías: <strong>{totalCampos}</strong>
                </span>
                <span style={styles.badge}>
                ...
                <Button
                  size="sm"
                  type="secondary"
                  style={{ borderRadius: 999, fontSize: 11, padding: '0 10px' }}
