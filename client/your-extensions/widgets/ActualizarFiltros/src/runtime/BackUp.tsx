...
function refreshYearCharts() {
  ...
  const cambiarSimbología = () => {
    ...
    if (attUnique) {
      ...
      for (const widget in chartWidgetsArray) {
        // console.log(campo)
        // console.log(chartWidgetsArray[widget].id)
        // console.log(masterChart[campo])
        // console.log(masterChart[campo][chartWidgetsArray[widget].id])
        // if (chartWidgetsArray[widget].id === 'widget_2') {
        //   console.log('makechart')
        //   console.log(makeChart(groupByWidgets[chartWidgetsArray[widget].id], campo, attUnique))
        ...
        if (
          ...
          getAppStore().dispatch(
            appActions.widgetConfigLoaded(
              ...
              // masterChart[campo][chartWidgetsArray[widget].id]
              makeChart(
                "pie",
  ...
  async function aplicarSimbologiaMapaTipoAccidenteSolo(
    ...
    if (!activarCluster) {
      ...
      // pon a true para usar tarta en clúster (mantiene paleta de colores automáticamente por categorías)
...
async function reiniciarTodo() {
  window.SimbologiaSeleccionada = "";

  // ➜ 1) limpiar primero (antes de tocar charts/mapa)
  await clearAllRuntimeFilters();
  // 👉 AÑADE ESTO: fuerza al WidgetUnido a “seleccionar TODO”
  pedirEliminarFiltros();
  // ➜ 2) reconfigurar charts + mapa
  cambiarSimbología();       // reinyecta configs al resto de charts
  await aplicarSoloMapa();   // aplica clúster por Tipo_accidente

  // ➜ 3) refresco específico de los charts de Año
  refreshYearCharts();

  // ➜ 4) resto como lo tenías
  ...
  // 👉 2) (Recomendado) Repite en el siguiente tick por si hubo re-montajes
  setTimeout(() => {
    getAppStore().dispatch(
  ...
  const reloadWidget = (widgetId: string) => {
    // Desactiva y reactiva el widget → se vuelve a montar
    getAppStore().dispatch(appActions.resetWidgetState(widgetId));
  };
function vaciarCampoEnManejoAtributos() {
  const idReceptor = 'widget_14'   // 👈 el ID del widget de la imagen
  const store = getAppStore()

  // Trigger para que el receptor se vacíe
  store.dispatch(
    appActions.widgetStatePropChange(idReceptor, 'resetCampoTrigger', Date.now())
  )

  // (opcional) además, forzar override explícito a vacío con nonce
  store.dispatch(
  ...
  /** id de la capa en el WebMap (preferido) */
  layerId?: string
  /** alternativamente, id del DataSource para resolver la capa */
  dataSourceId?: string
  /** título de leyenda/popup */
  legendTitle?: string
