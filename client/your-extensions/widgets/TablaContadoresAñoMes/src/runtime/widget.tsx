...
function getMonthNumber(value: any): number | undefined {
  ...
  const esIdx = ES_MONTHS_FULL.indexOf(s); if (esIdx >= 0) return esIdx + 1
  const enIdx = EN_MONTHS_FULL.indexOf(s); if (enIdx >= 0) return enIdx + 1

  // fechas
  if (/^\d{4}-\d{1,2}-\d{1,2}/.test(String(value)) || /^\d{1,13}$/.test(String(value))) {
    const d = new Date(asNum && String(value).length >= 11 ? asNum : value)
...
function getYearNumber(value: any): number | undefined {
  if (value == null || value === '') return undefined
  const asNum = Number(value)
  if (Number.isFinite(asNum) && asNum >= 1900 && asNum <= 9999) return Math.floor(asNum)
...
function colorToCss(c: any): string | undefined {
  ...
  if (Array.isArray(c)) {
    ...
    if (typeof (c as any).toJSON === 'function') {
      const j = (c as any).toJSON()
      const { r, g, b } = j; const a = j.a ?? 1
...
// ---------- Claves Año-Mes ----------
/** Devuelve {y,m} y una key "YYYY-MM" desde campos mes/año/fecha */
function getYearMonthFromRecord(r: Rec, monthField?: string, yearField?: string): { y?: number; m?: number; key?: string } {
  const y = yearField ? getYearNumber(r[yearField]) : getYearNumber(r[monthField as string])
  const m = getMonthNumber(r[monthField as string])
  if (y && m) return { y, m, key: `${y}-${pad2(m)}` }
  return {}
}

// ---------- Pivote Año-Mes ----------
function buildPivotByYearMonth(records: Rec[], monthField: string, typeField: string, yearField?: string) {
  // arma todas las claves presentes (tras definitionExpression)
  const keysSet = new Set<string>()
  records.forEach(r => {
    const { key } = getYearMonthFromRecord(r, monthField, yearField)
    if (key) keysSet.add(key)
  })
  const keys = Array.from(keysSet).sort((a, b) => (a > b ? 1 : -1)) // YYYY-MM orden cronológico
  ...
  const rows = types.map(t => {
    ...
    keys.forEach(k => {
      const cnt = records.reduce((acc, r) => {
        const ym = getYearMonthFromRecord(r, monthField, yearField).key
        return acc + (((r[typeField] ?? 'Sin valor') === t) && ym === k ? 1 : 0)
      }, 0)
      row[k] = cnt
      total += cnt
    })
...
const CustomAccidentPivotTable = (props: AllWidgetProps<any>) => {
  ...
  const [monthField, setMonthField] = React.useState<string>('') // Mes o Fecha
  const [yearField, setYearField] = React.useState<string>('')   // Año (opcional)
  ...
  React.useEffect(() => {
    ...
    if (byName) setMonthField(byName.name)
    else {
    ...
    // Año (si existe)
    const yCand =
      layer.fields.find((f: any) => /(año|anio|ano|year)$/i.test(f.name)) ||
      layer.fields.find((f: any) => /(año|anio|ano|year)$/i.test(f.alias || ''))
    if (yCand) setYearField(yCand.name)
    ...
    const runQuery = () => {
      ...
      q.where = layer.definitionExpression && layer.definitionExpression.trim() !== '' ? layer.definitionExpression : '1=1'
      q.outFields = ['*']
    ...
    runQuery()
    return () => { watcherDef?.remove?.(); watcherRend?.remove?.() }
  }, [jmvs])

  // construir tabla
  React.useEffect(() => {
    if (!tableRef.current) return
    if (tabRef.current) { tabRef.current.destroy(); tabRef.current = undefined }
    if (!records.length || !monthField || !typeField) {
      tableRef.current.innerHTML = '<p style="padding:8px">No hay datos para mostrar</p>'
      return
    }

    const { keys, rows } = buildPivotByYearMonth(records, monthField, typeField, yearField || undefined)
    ...
    const columns: any[] = [
      ...
      },
      {
        ...
        widthGrow: 0,
        headerMenu: [
          { label: 'Exportar a Excel', action: () => exportToExcelFromTabulator(tabRef.current, 'accidentes_anyo_mes.xlsx') }
        ]
      }
    ]
  ...
  }, [records, monthField, yearField, typeField, layerRef, colorResolver])
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-white" style={{ height: '100%' }}>
      ...
      <div className="diag-bar">
        <b>Agrupación:</b> <code>Año-Mes (YYYY-MM)</code> &nbsp;|&nbsp;
        <b>Campo Mes/Fecha:</b> <code>{monthField || '(sin resolver)'}</code> &nbsp;|&nbsp;
        <b>Campo Año:</b> <code>{yearField || '(no usa campo dedicado)'}</code> &nbsp;|&nbsp;
